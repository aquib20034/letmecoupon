                ...
                <tbody>
                    @foreach($sites as $key => $site)
                        <tr data-entry-id="{{ $site->id }}">
                            ...
                            <td>
                                <span style="display:none">{{ $site->publish ?? '' }}</span>
                            ...
                            <td>
                                ...
                                @can('site_delete')
                                    @php
                                        $url = isset(request()->test_id) ? route('admin.sites.destroy', $site->id) . '?test_id=1' : route('admin.sites.destroy', $site->id);
                                    @endphp
                                    <form action="{{ $url }}" id="{{ 'sites' . $site->id }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  $('.datatable-Site:not(.ajaxTable)').DataTable({ buttons: dtButtons })
