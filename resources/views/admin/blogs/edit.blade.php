...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="POST" action='{{ route("admin.blogs.update", [$blog->id]) }}' enctype="multipart/form-data" id="blogForm">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('author_id') ? 'is-invalid' : '' }}" name="author_id" id="author_id">
                    @foreach($authors as $id => $author)
                        <option value="{{ $id }}" {{ ($blog->author_id ? $blog->author->id : old('author_id')) == $id ? 'selected' : '' }}>{{ $author }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="stores">{{ trans('cruds.blog.fields.store') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('stores') ? 'is-invalid' : '' }}" name="stores[]" id="stores" multiple>
                    @foreach($stores as $id => $store)
                        <option value="{{ $id }}" {{ (in_array($id, old('stores', [])) || $blog->store_details->contains($id)) ? 'selected' : '' }}>{!! $store !!}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.store_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('meta_description') ? 'is-invalid' : '' }}" type="text" name="meta_description" id="meta_description" value="{{ old('meta_description', $blog->meta_description) }}" required maxlength="500" minlength="70">
                @if($errors->has('meta_description'))
                    <div class="invalid-feedback">
                        {{ $errors->first('meta_description') }}
                    </div>
                @endif
                <span class="help-block float-left">{{ trans('cruds.blog.fields.meta_description_helper') }}</span>
...
<script>
    ...
    if (characterCount >= 500) {
        ...
        $("#meta_description_message").html("{{ trans('cruds.blog.fields.meta_description_500') }}");
        ...
        current.css('fontWeight', 'bold');
        $("#meta_description_message").html("{{ trans('cruds.blog.fields.meta_description_320') }}");
