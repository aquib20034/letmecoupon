...
class CategoryApiController extends Controller
{
    ...
    public function index()
    {
        ...
        return new CategoryResource(Category::with(['sites', 'parent', 'user'])->get());
    }
    ...
    public function store(StoreCategoryRequest $request)
    {
        ...
        if ($request->input('icon', false)) {
            $category->addMediaFromUrl($request->input('icon'))->toMediaCollection('icon');
        }
    ...
    public function show(Category $category)
    {
        ...
        return new CategoryResource($category->load(['sites', 'parent', 'user']));
    }
    ...
    public function update(UpdateCategoryRequest $request, Category $category)
    {
        ...
        if ($request->input('icon', false)) {
            if (!$category->icon || $request->input('icon') !== $category->icon->file_name) {
                $category->addMedia(storage_path('tmp/uploads/' . $request->input('icon')))->toMediaCollection('icon');
            }
