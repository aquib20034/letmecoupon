        ...
        <div class="container">
                            ...
                            @if (!empty($detail['store_coupons']))
                                @foreach ($detail['store_coupons'] as $key => $coupon)
                                    @php
                                        ...
                                        if (str_contains($coupon['title'], '%')):
                                            $str = strtok($coupon['title'], '%');
                                            $num = (int) filter_var($str, FILTER_SANITIZE_NUMBER_INT);
                                        
                                            if ($num > 100):
                                                $finalVal = substr($num, 1);
                                            else:
                                                $finalVal = $num;
                                            endif;
                                        ...
                                        if (str_contains($coupon['title'], '$')):
                                            $avgSavings[] = $coupon['title'];
                                        endif;
                                        
                                        if ($coupon['code'] !== ''):
                                            $totalCodes[] = $coupon['id'];
                                        else:
                                            $totalDeals[] = $coupon['id'];
                                        endif;
                                    @endphp
                        ...
                        <div class="section">
                            ...
                            <div class="responsive-desc-border">
                                ...
                                <div class="description">
                                    <h4 class="top-heading left">{{ trans('sentence.more_about') }}
                                        {!! $detail['name'] !!}</h4>
                                    <div class="section ptn">
                                        <div class="coupon-table">
                                            <h5>{{ trans('sentence.about') }} {!! $detail['name'] !!}
            ...
            <div class="section short-column small no-padding sticky" id="storeHeaderWrp">
                <section class="sidebar-section">
                    <div class="store_rating">
                        <div class="image">
                            <picture>
                                <img src="{{ config('app.image_path') }}/build/images/placeholder.png"
                                    data-src="{{ isset($detail['store_image']) ? $detail['store_image'] : config('app.app_image') . '/build/images/placeholder.png' }}"
                        ...
                        <div class="rating-wrp">
                            <div class="rating-numbers-wrp">
                                <form class="rate">
                                    @if (isset($detail['rating']) && $detail['rating'] <= 5)
                                        ...
                                        @endfor
                                    @else
                                        <input type="radio" id="priceline1" name="priceline" value="13">
                                        <label class="full" for="priceline1">
                                            <i class="x_star"></i>
                                        </label>
                                        <input type="radio" id="priceline2" name="priceline" value="13">
                                        <label class="full" for="priceline2">
                                            <i class="x_star"></i>
                                        </label>
                                        <input type="radio" id="priceline3" name="priceline" value="13">
                                        <label class="full" for="priceline3">
                                        ...
                                        <input type="radio" id="priceline4" name="priceline" value="13">
                                        <label class="full" for="priceline4">
                                            <i class="x_star"></i>
                                        </label>
                                        <input type="radio" id="priceline5" name="priceline" value="13">
                                        <label class="full" for="priceline5">
                                            <i class="x_star"></i>
                                        </label>
                                    @endif
                            ...
                            <h1 class="title">{!! $detail['name'] !!}
                                {{ $site_wide_data['store_heading_one_suffix'] }}</h1>
                        </div>
                    </div>
                </section>
                        ...
                        <div class="chBxWr">
                            <span class="checkBox">
                                ...
                                <label for="coupon-box-sidebar">{{ trans('sentence.all') }}
                                    <span>({{ count($detail['store_coupons']) }})</span></label>
                            </span>
                        </div>
                        <div class="chBxWr">
                            <span class="checkBox">
                ...
                <div class="sidebar-section only-desktop">
                    <h2 class="secondary-heading left">{{ trans('sentence.about') }} {!! $detail['name'] !!}</h2>
                ...
                <div class="sidebar-section only-desktop">
                    <h2 class="secondary-heading left">{{ trans('sentence.todays_top') }} {!! $detail['name'] ?? '' !!}
                        {{ trans('sentence.top_promo_code') }}</h2>
                    <div class="sidebar-committed">
                        <ul>
                            <li><span>{{ trans('sentence.total_offers_sidebar') }}</span><i>{{ count($detail['store_coupons']) }}</i>
                            </li>
                            ...
                            @php
                                if (sizeof($avgSavings) > 0) {
                                    ...
                                    foreach ($avgSavings as $v) {
                                        $ex = explode("$", $v);
                                        $value = filter_var($ex[1], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        if (!empty($value)) {
                                            $val[] = $value;
                                        }
                                    }
