...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive" id="datatable-Network">
...
@section('scripts')
    @parent
    @php
        $url = isset(request()->test_id) ? route('admin.networks.index') . '?test_id=1' : route('admin.networks.index');
        if(isset(request()->nid) && isset(request()->test_id)) {
            $url = $url . "&nid=" . request()->nid;
        } elseif (isset(request()->nid)) {
            $url = $url . "?nid=" . request()->nid;
        }
    @endphp
    <script>
        ...
        $(function () {
            ...
            $('.datatable-Network').DataTable(dtOverrideGlobals);
            ...
            document.getElementById('siteOpt').onchange = function () {
                $("#datatable-Network").html("");
                $("#datatable-Network").html(`
                        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Network">
                            <thead>
                                <tr>
                                    <th width="10">
